/* ============================================
   ...
   Versione rifatta da zero - 3 file
   - Tabella Clienti
   - Sum_of / Sottoprodotti
   - STATI Pratiche create nell'anno
   - Avanzamento Clienti
   - Analisi TC
...
function updateRunEnabled() {
  if (btnRun) btnRun.disabled = !(state.tabOk && state.sumOk && state.statiOk);
}
function showFileStatus(kind, text = "Verifica in corso...") {
  const el = kind === "tab" ? statusTab : (kind === "sum" ? statusSum : statusStati);
  if (!el) return;
...
async function onFileSelected(kind) {
  ...
  const input = kind === "tab" ? fileTab : (kind === "sum" ? fileSum : fileStati);
...
def score(cols):
    ...
    # STATI Pratiche: il file può avere intestazioni leggermente diverse.
    if "RESPONSABILE" in cset:
        stati_s += 4
    if any(("COMMESSA" in c or "PRATICA" in c) for c in cols):
        stati_s += 4
    if any("TIPOSOGGETTO" in c or "TIPOCLIENTE" in c or "SOGGETTO" in c for c in cols):
        stati_s += 2
    for b in ["PRODOTTOLORDO", "PRODOTTO", "FATTURATOTOTALE", "FATTURATO", "INCASSATOTOTALE", "INCASSATO", "VENDUTO"]:
        if any(b in c for c in cols):
            stati_s += 2
...
try:
    ...
    for i in range(min(100, len(df0))):
        ...
        is_stati = (
            "RESPONSABILE" in row
            and any(("COMMESSA" in c or "PRATICA" in c) for c in row)
            and any(("PRODOTTO" in c or "FATTURATO" in c or "INCASSATO" in c or "VENDUTO" in c or "SOGGETTO" in c) for c in row)
        )
...
`);
    ...
    const expected = kind === "tab" ? "tabella" : (kind === "sum" ? "sumof" : "stati");
    ...
    if (okText) {
      okText.textContent = kind === "tab"
        ...
        : (kind === "sum" ? "Sum_of verificato." : "STATI Pratiche verificato.");
    }
    showModal(okModal);
...
def find_header_row(xlsx_bytes, expected_type):
    ...
    for i in range(min(100, len(df0))):
        ...
        if expected_type == "sumof" and "ANNO" in row and "MESE" in row and "CODICESOGGETTO" in row:
            return i
        if expected_type == "stati" and "RESPONSABILE" in row and any(("COMMESSA" in c or "PRATICA" in c) for c in row):
            return i
    return 0
...
def read_excel_robust(xlsx_bytes, expected_type):
    try:
        ...
        if expected_type == "sumof" and "CODICESOGGETTO" in cols and "ANNO" in cols:
            return df
        if expected_type == "stati" and "RESPONSABILE" in cols and any(("COMMESSA" in c or "PRATICA" in c) for c in cols):
            return df
    except Exception:
        pass
...
def _parse_tc_name(x):
    """
    Restituisce:
    - key: identificativo tecnico univoco, es. C.TARTAMELLA
    - initial: prima lettera, es. C
    - surname: cognome, es. TARTAMELLA

    Obiettivo:
    - se il cognome è unico -> TARTAMELLA
    - se ci sono due persone con stesso cognome -> C. TARTAMELLA / P. TARTAMELLA
    """
    if pd.isna(x):
        return "", "", ""
    ...
    surname = surname.replace("_", " ").replace("-", " ").strip().upper()
    surname = re.sub(r"\s+", " ", surname)

    key_base = local.replace("_", ".").replace("-", ".")
    key_base = re.sub(r"\s+", "", key_base).upper()
    key = key_base if key_base else surname

    return key, initial, surname


def build_tc_label_map(sources):
    records = {}
    surname_to_keys = {}
    ...
    for source in sources:
        if source is None:
            continue
        try:
            values = source.dropna().tolist()
        except Exception:
            values = list(source)

        for value in values:
            key, initial, surname = _parse_tc_name(value)
            if not key or not surname:
                continue

            if key not in records:
                records[key] = {"initial": initial, "surname": surname}

            surname_to_keys.setdefault(surname, set()).add(key)

    label_map = {}
    for key, info in records.items():
        surname = info["surname"]
        initial = info["initial"]
        duplicate_surname = len(surname_to_keys.get(surname, set())) > 1

        if duplicate_surname and initial:
            label_map[key] = f"{initial}. {surname}"
        else:
            label_map[key] = surname

    return label_map


def format_tc_name(x, tc_label_map=None):
    key, initial, surname = _parse_tc_name(x)
    if not key or not surname:
        return ""
    if tc_label_map is not None and key in tc_label_map:
        return tc_label_map[key]
    return surname
...
# =========================
# STATI PRATICHE - Prodotto / Fatturato / Incassato
# =========================
st_resp = pick_col(stati, ["RESPONSABILE"], fallback_idx=6)
st_comm = pick_col(stati, ["CODICECOMMESSA", "CODICE COMMESSA", "COMMESSA", "CODICEPRATICA", "CODICE PRATICA"], fallback_idx=8)
st_tipo = pick_col(stati, ["TIPOSOGGETTO", "TIPO SOGGETTO", "TIPO CLIENTE", "SOGGETTO"], fallback_idx=10)
...
stati_df = stati_df[~stati_df["Responsabile"].str.lower().str.contains("totale", na=False)].copy()
stati_df["_row"] = np.arange(len(stati_df))

# Se lo stesso CodiceCommessa compare più volte, tengo l'ultima riga disponibile.
stati_df = (
...
# Mappa nomi TC:
# di default mostra solo il cognome;
# se esistono due persone con lo stesso cognome, aggiunge l'iniziale.
# Esempio: c.tartamella e p.tartamella -> C. TARTAMELLA / P. TARTAMELLA
tc_label_map = build_tc_label_map([
    clients["Referente_Commerciale"] if "Referente_Commerciale" in clients.columns else [],
    sumdf["Chi"] if "Chi" in sumdf.columns else [],
    stati_df["Responsabile"] if "Responsabile" in stati_df.columns else [],
])

stati_df["TC"] = stati_df["Responsabile"].apply(lambda x: format_tc_name(x, tc_label_map))
...
for _col_nome in [
    ...
    if _col_nome in avanzamento_clienti.columns:
        avanzamento_clienti[_col_nome] = avanzamento_clienti[_col_nome].apply(lambda x: format_tc_name(x, tc_label_map))
...
clients_tc["TC"] = clients_tc["Referente_Commerciale"].apply(lambda x: format_tc_name(x, tc_label_map))
...
sumdf_tc["TC"] = sumdf_tc["Chi"].apply(lambda x: format_tc_name(x, tc_label_map))
...
if len(delib_tc):
    delib_tc["TC"] = delib_tc["Chi"].apply(lambda x: format_tc_name(x, tc_label_map))
...
# Prodotto / Fatturato / Incassato dal file STATI Pratiche.
stati_all = (
...
avanzamento_tc["TC"] = avanzamento_tc["Referente_Commerciale"].apply(lambda x: format_tc_name(x, tc_label_map))
...
regole_ra = pd.DataFrame([
    ...
    ["FOGLIO ANALISI_TC", "Riepiloga per TC preventivi, delibere, venduto, prodotto, fatturato, incassato, percentuali sugli amministratori, chiusura e stato portafoglio."],
...
with pd.ExcelWriter(out, engine="openpyxl") as writer:
    ...
    if "Analisi_TC" in wb.sheetnames:
        ...
        euro_headers = {
            "VENDUTO", "VENDUTO AMM.",
            "DELIBERA MEDIA", "DELIBERA MEDIA AMM.",
            "PRODOTTO", "PRODOTTO AMM.",
            "FATTURATO", "FATTURATO AMM.",
            "INCASSATO", "INCASSATO AMM."
        }
        pct_headers = {
            "% PREV. AMM.", "% DEL. AMM.", "% VENDUTO AMM.",
            "% CHIUSURA", "% CHIUSURA AMM.",
            "% PRODOTTO AMM.", "% FATT. AMM.", "% INC. AMM."
        }
        for c_idx, h in enumerate(header, start=1):
