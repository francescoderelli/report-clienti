...
def score(cols):
    ...
    for m in ["RESPONSABILE", "CODICECOMMESSA", "TIPOSOGGETTO"]:
        if m in cset:
            stati_s += 3
    for b in ["PRODOTTOLORDO", "FATTURATOTOTALE", "INCASSATOTOTALE", "VENDUTO"]:
        if any(b in c for c in cols):
            stati_s += 2
...
def find_header_row(xlsx_bytes, expected_type):
    ...
    for i in range(min(100, len(df0))):
        ...
        if expected_type == "sumof" and "ANNO" in row and "MESE" in row and "CODICESOGGETTO" in row:
            return i
        if expected_type == "stati" and "RESPONSABILE" in row and "CODICECOMMESSA" in row and "TIPOSOGGETTO" in row:
            return i
    return 0
